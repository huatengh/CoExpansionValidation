...
BayeSSC is used for simulation, and msReject is used for inference. While functions in this package do not directly use [hBayeSSC](https://github.com/UH-Bioinformatics/hBayeSSC), steps of hyperstat calculation and running msReject are set to be identical to [hBayeSSC](https://github.com/UH-Bioinformatics/hBayeSSC). Future development will expand on other types of summary statistics.
...
* user specifies the _alpha_ for Dirichlet process, and simulates the number of co-expansion events  
...
For the prior on the degree of concordance, hBayeSSC used a flat, uniform distribution. Here, we adopted the [PyMsbayes-style](http://joaks1.github.io/PyMsBayes/) prior, using a gamma distribution for the alpha parameter of the [dirichelet process](http://phyletica.org/dirichlet-process/). User needs to decide on the two parameters for the gamma distribution: concentrationShape and consentrationScale. Check [prior selection in PyMsbayes](http://joaks1.github.io/PyMsBayes/tutorials/selecting-priors.html) for how to select these two parameters.  
...
With the posterior file after running msReject, we can use the VGAM, abc and locfit package to do the final acceptance and parameter estimation, some scripts are on the [hBayeSSC webpage](https://github.com/UH-Bioinformatics/hBayeSSC#msreject-module). Here is some code for looking at the mode of event number in the posterior samples.
